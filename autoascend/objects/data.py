...
Weapon = namedtuple('Weapon',
                    'name,desc,kn,mg,bi,prob,wt,cost,sdam,ldam,hitbon,typ,sub,metal,color,damage_small,damage_large'.split(
                        ','))
Armor = namedtuple('Armor', 'name,desc,kn,mgc,blk,power,prob,delay,wt,cost,ac,can,sub,metal,c'.split(
    ','))  # ac is always <= 0, i.e. ac = original_ac - 10
...
Container = namedtuple('Container', 'name,desc,kn,mgc,chg,prob,wt,cost,mat,color'.split(','))
WepTool = namedtuple('WepTool', 'name,desc,kn,mgc,bi,prob,wt,cost,sdam,ldam,hitbon,sub,mat,clr'.split(','))
...
#           nutrition, color)
def FOOD(*args):
    return Food(args[0], None, *args[1:])
...
objects = [
    ...
    FOOD("meat ring", 0, 1, 5, 0, FLESH, 5, CLR_BROWN),
    ...
    SPELL_LEATHER("dig", "parchment",
                  P_MATTER_SPELL, 20, 6, 5, 1, RAY, HI_LEATHER),
    SPELL_LEATHER("magic missile", "vellum",
...
for name, _, damage_small, damage_large in weapon_damage:
    for i in range(len(objects)):
        if objects[i] is None or not isinstance(objects[i], Weapon):
            continue
